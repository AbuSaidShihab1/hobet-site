...
function PaymentCallbackPage() {
  ...
  const payment_status=()=>{
    axios.post(`http://localhost:8080/user/create-transaction`,{payment_type:"Deposit",amount:amount,payment_method:transaction_info.provider,status:status === "cancel" ? "failed" : status,customer_id:user_info._id})
    .then((res)=>{
      console.log(res)
    }).catch((err)=>{
      console.log(err)
